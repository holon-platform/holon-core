...
/**
 * Filter which represents a "IS GREATER (OR EQUAL)" predicate.
 * 
 ...
 * @see QueryFilter
 */
public class GreaterFilter<T> extends AbstractOperationQueryFilter<T> {

	private static final long serialVersionUID = -8425117091987015333L;

	/**
	 * Constructor.
	 * @param left Left operand
	 * @param right Right operand
	 * @param includeEquals if <code>true</code>, build a GREATER OR EQUAL filter condition
	 */
	public GreaterFilter(QueryExpression<T> left, QueryExpression<? super T> right, boolean includeEquals) {
		super(left, includeEquals ? FilterOperator.GREATER_OR_EQUAL : FilterOperator.GREATER_THAN, right);
	}
	...
	public boolean isIncludeEquals() {
		return getOperator() == FilterOperator.GREATER_OR_EQUAL;
	}
