...
* The new `holon-spring-security` artifact provides integration between the Holon Platform authentication architecture and the Spring Security `SecurityContext`. Furthermore, a `holon-starter-security` Spring Boot starter is now available for quick project setup. See <<SpringSecurity>>.
